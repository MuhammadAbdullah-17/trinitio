...
class Timezone extends React.Component {
  ...
  getTimezone(query) {
    ...
    return Object.keys(this.props.timezones).find(
      (zone) => zone === query || this.props.timezones[zone] === query
    );
  }
  ...
  filterItems(filter) {
    ...
    Object.keys(timezones).forEach((key) => {
      if (
        this.constructor.zoneCompare(key, filter) ||
        this.constructor.zoneCompare(timezones[key], filter)
      ) {
    ...
    if (value) {
      ...
      for (let i = 0; i < keyz.length; ++i) {
        if (keyz[i] === value || timezones[keyz[i]] === value) {
          this.setState({ isOpen: true, focused: i }, () => {
  ...
  handleKeyPress(e) {
    ...
    if (e.which === UP_KEY || e.which === DOWN_KEY) {
      ...
      if (e.which === UP_KEY) {
        newFocused =
          focused === 0 ? Object.keys(timezones).length - 1 : focused - 1;
      } else {
        newFocused =
          focused === Object.keys(timezones).length - 1 ? 0 : focused + 1;
      }
  ...
  render() {
    ...
    const containerClasses = classnames("timezone-picker", className, {
    ...
    const listClass = classnames(
      ...
      `timezone-picker-list-${absolute ? "abs" : "rel"}`
    );
...
Timezone.defaultProps = defaultProps;
